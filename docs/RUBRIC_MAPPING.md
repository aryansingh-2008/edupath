...
### 3. AI Integration (25%)
- **Agent Roles**:
  1. `Profile Agent`: Parses unstructured resume text and outputs verified capabilities + evidence quotes.
  2. `Skill Intelligence Agent`: Normalizes skill synonyms to canonical ontology.
  3. `Gap Agent`: Computes vector distance and scalar delta between user and role benchmarks.
  4. `Roadmap Planner Agent`: Constructs dependency DAG of weekly objectives.
  5. `Resource Agent`: Ranks curated, verified learning materials.
  6. `Practice Agent`: Generates calibrated diagnostic questions matching the learner's zone of proximal development.
  7. `Progress Agent`: Updates skill scores based on practice accuracy.
  8. `Adaptive Planner Agent`: Detects consecutive failures, triggers route recalculation, and generates explainability receipts.
  9. `Mentor Agent`: Ingests active profile context to provide personalized guidance.
